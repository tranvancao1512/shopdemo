...
class THEMECOMPLETE_EPO_Cart {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_add_to_cart', array( $this, 'woocommerce_add_to_cart' ), 12, 6 );
		...
		add_action( 'woocommerce_add_to_cart', array( $this, 'edit_woocommerce_add_to_cart' ), 10, 6 );
	...
	public function add_cart_item( $cart_item = array() ) {
		...
		*/
		...
		if ( floatval( apply_filters( 'tm_epo_no_zero_priced_products', floatval($cart_item['tm_epo_product_price_with_options']) + $associated_products_price, $cart_item ) ) == 0 ) {
				...
				if ( isset( $section['other_data'] ) && is_array( $section['other_data'] ) ) {
					foreach ( $section['other_data'] as $key => $data ) {
						...
						if ( ! empty( $data['images'] ) && THEMECOMPLETE_EPO()->tm_epo_show_image_replacement == "yes" ) {
							...
							$quantity_string       = ( $data['quantity'] > 1 ) ? ' &times; ' . $data['quantity'] : '';
							$display_other_value_only = '<span class="cpf-img-on-cart"><img alt="' . esc_attr( strip_tags( $section['label'] ) ) . '" class="attachment-shop_thumbnail wp-post-image epo-option-image" src="' .
													apply_filters( "tm_image_url", $data['images'] ) . '" />';
						...
						if ( THEMECOMPLETE_EPO()->tm_epo_show_hide_uploaded_file_url_cart == "no" && THEMECOMPLETE_EPO()->tm_epo_show_upload_image_replacement == "yes" && $section['type'] == "upload" ) {
							...
							if ( ! empty( $check['ext'] ) ) {
								...
								if ( in_array( $check['ext'], $image_exts ) ) {
									$display_other_value_only = '<span class="cpf-img-on-cart"><img alt="' . esc_attr( strip_tags( $section['label'] ) ) . '" class="attachment-shop_thumbnail wp-post-image epo-option-image epo-upload-image" src="' .
	 ...
	 * @return string|void
	 */
	public function edit_woocommerce_add_to_cart( $cart_item_key = "", $product_id = "", $quantity = "", $variation_id = "", $variation = "", $cart_item_data = "" ) {
