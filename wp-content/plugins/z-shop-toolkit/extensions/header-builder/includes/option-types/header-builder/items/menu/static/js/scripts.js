fwEvents.one('fw-builder:' + 'header-builder' + ':register-items', function (builder) {
	var localized = header_builder_item_type_menu;

	var ItemView = builder.classes.ItemView.extend({
		template: _.template(
			'<div class="fw-header-builder-item-style-default fw-header-builder-item-type-menu <%- center_el %>">' +
			...
			/**//**/'<i class="ion-android-menu" style="<%- style %>"></i>' +
			/**/'</div>' +
			'</div>'
		),
		events: {
			'click': 'openEdit',
			'click .fw-header-builder-item-type-menu': 'openEdit',
	...
	var Item = builder.classes.Item.extend({
		...
		initialize: function () {
			...
			this.view = new ItemView({
				id: 'fw-builder-menu-item-' + this.cid,
				model: this
			});
		},
