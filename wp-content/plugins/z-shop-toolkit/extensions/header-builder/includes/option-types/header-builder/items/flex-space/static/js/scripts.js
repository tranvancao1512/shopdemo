fwEvents.one('fw-builder:' + 'header-builder' + ':register-items', function (builder) {
	var localized = header_builder_item_type_flex_space;

	var ItemView = builder.classes.ItemView.extend({
		template: _.template(
			'<div class="fw-header-builder-item-style-default fw-header-builder-item-type-flex-space">' +
		...
		events: {
			// 'click': 'openEdit',
			// 'click .fw-header-builder-item-type-flex-space': 'openEdit',
		...
		render: function () {
			...
			// 	this.$('.fw-header-item-width').append(
	...
	var Item = builder.classes.Item.extend({
		...
		initialize: function () {
			...
			this.view = new ItemView({
				id: 'fw-builder-flex-space-item-' + this.cid,
				model: this
			});
		},
