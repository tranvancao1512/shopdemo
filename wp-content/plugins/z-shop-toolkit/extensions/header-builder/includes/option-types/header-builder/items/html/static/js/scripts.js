fwEvents.one('fw-builder:' + 'header-builder' + ':register-items', function (builder) {
	var localized = header_builder_item_type_html;

	var ItemView = builder.classes.ItemView.extend({
		template: _.template(
			'<div class="fw-header-builder-item-style-default fw-header-builder-item-type-html <%- center_el %>">' +
		...
		events: {
			'click': 'openEdit',
			'click .fw-header-builder-item-type-html': 'openEdit',
		...
		render: function () {
			var renderData = {
				...
				html: fw.opg('html', this.model.get('options')),
			...
			if( ! renderData.html ) {
				renderData.html = localized.l10n.placeholder;
			}
